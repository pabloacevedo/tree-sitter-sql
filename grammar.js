  ...
  rules: {
    program: ($) =>
      seq(
        ...
        repeat(
          seq(
            choice($.transaction, $.statement, $.block),
            choice(";", $.keyword_go)
          )
        ),
    ...
    transaction: ($) =>
      seq(
        ...
        repeat(seq($.statement, choice(";", $.keyword_go))),
    ...
    block: ($) =>
      prec.right(
        seq(
          $.keyword_begin,
          optional(choice(";", $.keyword_go)),
          repeat(seq(choice($.statement, $.block), choice(";", $.keyword_go))),
          $.keyword_end,
          optional(choice(";", $.keyword_go))
        )
      ),
    ...
    storage_parameters: ($) =>
      seq(
        $.keyword_with,
        paren_list(seq($.identifier, optional(seq("=", $.literal))), true)
      ),
    ...
    declare_statement: ($) =>
      seq($.keyword_declare, optional("@"), $.identifier, $._type),
    ...
    procedure_body: ($) => choice($.statement, $.block),
    ...
    _function_body_statement: ($) =>
      choice(
        $.statement,
        $._function_return,
        seq($.keyword_return, $.keyword_query, $.statement)
      ),

    function_body: ($) =>
      choice(
        seq($._function_return, choice(";", $.keyword_go)),
        seq(
          $.keyword_begin,
          $.keyword_atomic,
          repeat1(seq($._function_body_statement, choice(";", $.keyword_go))),
          $.keyword_end
        ),
        seq(
          $.keyword_as,
        ...
        repeat(
          choice(
            ...
            seq(
              ...
              // mariadb
              optional(
                seq(choice($.keyword_follows, $.keyword_precedes), $.identifier)
              )
            ),
    ...
    object_reference: ($) =>
      seq(
        optional(seq(optional_brackets(field("schema", $.identifier)), ".")),
        optional_brackets(field("name", $.identifier))
      ),
    ...
    insert: ($) =>
      seq(
        ...
        choice($._insert_values, $._set_values, $.execute_procedure),
        optional(
          seq(
            $.keyword_on,
            $.keyword_conflict,
    ...
    _default_constraint: ($) => seq($.keyword_default, $._expression),
    ...
    ordered_column: ($) =>
      seq(optional_brackets(field("name", $._column)), optional($.direction)),
    ...
    cross_join: ($) => seq($.keyword_cross, $.keyword_join, $.relation),
...
function optional_brackets(node) {
  return prec.right(choice(node, seq("[", node, "]")));
}
